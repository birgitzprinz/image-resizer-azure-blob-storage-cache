...
namespace ImageResizer.Plugins.AzureBlobStorageCache.Async
{
    public class AsyncWriteCollection
    {
        ...
        /// <returns></returns>
        public AsyncWrite Get(string relativePath)
        {
            lock (_sync)
            {
                AsyncWrite result;
                return c.TryGetValue(relativePath, out result) ? result : null;
            }
        }
        ...
        public bool Queue(AsyncWrite w, WriterDelegate writerDelegate)
        {
            lock (_sync)
            {
                ...
                if (c.ContainsKey(w.Path)) return false; //We already have a queued write for this data.
